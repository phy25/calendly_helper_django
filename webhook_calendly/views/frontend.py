...
def student_reports(request: HttpRequest):
    ...
    if latest_spot_booking:
        ...
        #group_last_approved_subquery = Subquery(Booking.objects.filter(
        #    calendly_data__approval_group__id=OuterRef('id'),
        #    approval_status=Booking.APPROVAL_STATUS_APPROVED
        #).values('spot_start', 'event_type_id').order_by('-booked_at')[:1])
        ...
        groups_list = ApprovalGroup.objects.filter(
            ...
            #last_approved_spot_start=F('approved_booking__booking__spot_start'),
            #last_approved_spot_start=group_last_approved_subquery,
            #last_approved_event_type_id=group_last_approved_subquery[1],
            #declined_bookings_total=Subquery(Booking.objects.filter(
            #    calendly_data__approval_group__id=OuterRef('id'),
            #    approval_status=Booking.APPROVAL_STATUS_DECLINED
            #).values('spot_start').order_by('-booked_at')[:1])
        #)
